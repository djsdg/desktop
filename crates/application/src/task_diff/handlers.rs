    ...
    pub fn handle(
        ...
        if message.is_empty() {
            return Err(ApplicationError::TaskDiff {
                message: "commit message must not be blank".to_string(),
            });
        }
        let task_id = TaskId::new(request.task_id);
        let (task, worktree) =
...
fn recorded_branch(worktree: &Worktree) -> Result<&str, ApplicationError> {
    worktree
        ...
        .ok_or_else(|| ApplicationError::TaskDiff {
            message: "task worktree has no recorded branch".to_string(),
        })
}
...
fn task_git_writer_error(error: super::TaskGitWriterError) -> ApplicationError {
    match error {
        super::TaskGitWriterError::OperationFailed(message) => {
            ApplicationError::TaskDiff { message }
        }
    }
}
